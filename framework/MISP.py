...
class MISP(base):
    ...
    def __init__(self):
        ...
        self.spendable_parking_slots_matrix = np.zeros((len(self.location), 24)) # row: locationId, column: 0~23 (hour)
        self.usage_slots_matrix = np.zeros((len(self.location), 24)) # 已使用的充電為數量
    ...
    def set_budgets(self):
        ...
        這裡可以改成用"歷史超過契約容量"的比例去配
        '''
        locationId_index = self.location.index
        self.budget = pd.Series([0 for _ in range(20)], index=locationId_index, name="budget")
        ratio_series = pd.Series([0 for _ in range(20)], index=locationId_index, name="ratio")
        ...
        for _, row in location_avg_request_nums_df.iterrows():
            ...
            ratio_series[locationId] = (location_avg_request_nums_df["num"].max() + 1) / (row["num"] + 1)

        unit = INCENTIVE_BUDGET / ratio_series.sum()

        for _, row in location_avg_request_nums_df.iterrows():
            locatonId = row["locationId"]
            self.budget[locatonId] = math.floor(unit * ratio_series[locatonId])
    ...
    def set_threshold(self, combinations, date):
        ...
        locationID_index = self.budget.index
        threshold = pd.Series([0 for i in range(20)], index=locationID_index, name="threshold") # 每個建物的門檻值

        for locationID in locationID_index:
            ...
            if location_budget == 0:
                threshold[locationID] = self.threshold_max
                continue
            
            threshold[locationID] = ((max_score + min_score) * average_request) / (2 * location_budget)
        ...
        if first:
            ...
            for csID in locationId_list:
                for hour in range(24):
                    ...
                    # 初始化使用率
                    self.load_matrix[int(buildingID)][hour] = 0 if self.spendable_parking_slots_matrix[int(buildingID)][hour] != 0 else 1 # 初始化的時候，沒有可用空位代表使用率 100% 
    ...
    def expected_score(self, user_list, userID, csID, hour, incentive):
        ...
        buildingID = int(self.location.loc[csID, "buildingID"])
        load_value = self.average_utilization_ratio[hour] - self.load_matrix[buildingID][hour] \
                    / self.average_utilization_ratio.max() \
                    if self.average_utilization_ratio.max() != 0 else 1
    ...
    def initial_filter(self, combinations):
        ...
        for _, schedule in enumerate(combinations):
            ...
            if ((schedule[2] / schedule[3]) >= self.threshold.loc[schedule[0]]) and \
                (self.incentive_cost.index(schedule[3]) <= self.budget[schedule[0]]):
    ...
    for random_counter in range(random_start_counter, random_end_counter+1):
        ...
        for day in range(7):
            ...
            for rID, userID, charging_len, origin_hour, origin_cs in charging_request:
                try:
                    recommend = misp.OGAP(user_list, slots_df, userID, int(charging_len))
                    ### User select ###
