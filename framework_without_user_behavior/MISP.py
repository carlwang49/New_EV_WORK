...
### global variable ###
ALPHA = 0.5
TESTING_NAME = "MISP_without_userBehavior"
TEST_DAY = "2023-06-28"
...
class MISP(base):
    ...
    def set_budgets(self):
        '''
        設定每個充電站的預算
        (依照不同充電站歷史請求數量分配，數量越多分配到的越少 --> cs_i_budget = (max_request/request_i)/sum(max_request/request_i) * total_budget)
    ...
    def OGAP(self, user_list, slots_df, userID, charging_len):
        ...
        if len(combinations) == 0:
            ...
            return self.default_recommend(self.user_preference[userID], userID, charging_len) # 不需要給用戶原本 request 的充電開始時間, combinations: (csID, hour, score, schedule_cost)
