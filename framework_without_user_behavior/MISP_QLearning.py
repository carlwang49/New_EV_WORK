...
### global variable ###
ALPHA = 0.5
TESTING_NAME = "MISP_Q_Learning_without_userBehavior"
TEST_DAY = "2023-06-28"
...
class QLearn(MISP):
    ...
    def get_q_learning_recommend(self, userID, combinations, q_table):
        ...
        index_option_dict = self.filter_q_value(q_table_list) # q_value 一樣，但 score 最大的
        index_option_list = list(index_option_dict.items())
        sort_score = sorted(index_option_list, key=lambda x: x[1][2], reverse=True) # q_value 由大排到小，根據 score
    ...
    for day in range(7):
        ...
        for rID, userID, charging_len, origin_hour, origin_cs in charging_request:
            ...
            try:
                ...
                if q_learn_index == 0:
                    user_choose = model.choose_recommend(q_learn_recommend)
